...
class JobScheduleRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setDueAt($dueAt)
    {
        ...
        if (!is_null($dueAt) && (!preg_match("/^((0[1-9]|[12]\\\\d|3[01])\\\/(0[1-9]|1[0-2])\\\/[12]\\\\d{3}) ([01]?\\\\d|2[0-3]):([0-5]?\\\\d):([0-5]?\\\\d)$/", $dueAt))) {
            throw new \InvalidArgumentException("invalid value for $dueAt when calling JobScheduleRequest., must conform to the pattern /^((0[1-9]|[12]\\\\d|3[01])\\\/(0[1-9]|1[0-2])\\\/[12]\\\\d{3}) ([01]?\\\\d|2[0-3]):([0-5]?\\\\d):([0-5]?\\\\d)$/.");
        }
